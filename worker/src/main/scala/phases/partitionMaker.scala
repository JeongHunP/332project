...
object partitionMaker {
  ...
  def writeItems(
    ...
    val filename = outputDir + "/To_" + to.toString() + "_From_" + from
      .toString() + "_" + count.toString
  ...
  def partitionSingleFile(
    ...
    def divideItems(item: Seq[String], index: Int): Unit = {
      ...
      if (
        findRange(
          item(index),
          ranges(rangeIndex).from,
          ranges(rangeIndex).to
        ) == false
      ) {
    ...
    for (i <- 0 to filePaths.length - 1) {
      var sorteditems =
        Source
          ...
          .getLines()
          .toStream // Utils.getFile(filePaths(i))
